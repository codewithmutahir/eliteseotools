  ...
  const handleFileSelect = async (selectedFile: File) => {
    ...
    try {
      ...
      if (!data.serverSideAvailable) {
        console.info(
          "Server-side conversion not available. Using client-side conversion."
        );
      }
    } catch (error: any) {
      ...
      alert(
        error.message ||
          "Failed to load PDF. Please check if the file is a valid PDF."
      );
    } finally {
      setLoading(false);
    }
      ...
      try {
        ...
        for (const pageNum of selectedPages.sort((a, b) => a - b)) {
          ...
          if (useClientSide) {
            ...
            try {
              blob = await convertPdfPageToImage(
                file,
                pageNum,
                imageFormat,
                parseFloat(scale)
            ...
            if (!response.ok) {
              ...
              if (
                errorData.useClientSide ||
                errorData.requiresCanvas ||
                !useClientSide
              ) {
                ...
                try {
                  ...
                  throw new Error(
                    `Failed to convert page ${pageNum}: ${
                      clientError.message || "Conversion failed"
                    }`
                  );
                }
              } else {
                throw new Error(
                  `Unable to convert page ${pageNum}. Please try again.`
                );
              }
            } else {
        ...
        let errorMessage =
          "Unable to convert pages. Please try again or check your PDF file.";
        ...
        if (error.message) {
          ...
          if (
            ...
            errorMessage =
              "Your browser doesn't support image conversion. Please try a different browser.";
          ...
          >
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={converting || selectedPages.length === 0}
              >
                {converting
                  ? "Converting..."
                  : `Convert ${
                      selectedPages.length
                    } Page(s) to ${imageFormat.toUpperCase()}`}
              </Button>
